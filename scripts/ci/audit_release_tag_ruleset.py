...
def validate_live_release_tag_ruleset(
    document: object,
    *,
    expected_repository: str,
) -> list[str]:
    if not isinstance(document, dict):
        return ["live release-tag ruleset must be a JSON object"]
    ...
    # GitHub may omit bypass_actors for callers without Ruleset write access.
    # The read-only doctor validates it when present and otherwise leaves the
    # bypass check to the administrator import review documented for this profile.
    canonical.setdefault("bypass_actors", [])

    errors.extend(validate_release_tags(canonical))
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 2:
        print(
            "Usage: audit_release_tag_ruleset.py <ruleset-json|-> owner/repo",
            file=sys.stderr,
        )
        return 2
    ...
    print(
        f"Live Ruleset {EXPECTED_NAME!r} matches the immutable release-tag contract "
        f"for {repository}."
    )
