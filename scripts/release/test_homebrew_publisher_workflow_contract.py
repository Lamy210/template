...
class HomebrewPublisherWorkflowContractTests(unittest.TestCase):
    ...
    def test_published_assets_are_rebound_before_cask_render(self) -> None:
        ...
        self.assertIn("APP_NAME: ${{ inputs.app_name }}", block)
        self.assertIn("BUNDLE_ID: ${{ inputs.bundle_id }}", block)
        self.assertIn('--app-basename "${APP_NAME}.app"', block)
        self.assertIn('--bundle-id "${BUNDLE_ID}"', block)
