...
10. queries the automation branch fail-closed, rebuilds it from the trusted tap default branch, records the exact fetched default-branch commit, and immediately before every branch push requires the remote default branch to still equal that observed commit as well as re-resolving canonical repository/default-branch metadata; only then does it push with an explicit force-with-lease expectation (including a create-only empty lease when the branch was absent);
11. when the rendered Cask is already current but an old automation branch exists, requires the local cleanup commit to equal that observed default-branch commit before mutation, resets the automation branch with the same pre-push default-head rebind, and then makes a final two-ref remote query requiring both the default and automation refs to still equal the cleanup SHA;
