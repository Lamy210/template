...
def canonical_verified_release_artifact_name(
    publisher_run_id: int,
    publisher_run_attempt: int,
) -> str:
    return f"verified-macos-release-{publisher_run_id}-{publisher_run_attempt}"
...
def verify_verified_release_artifact(
    *,
    ...
    if not isinstance(artifact_digest, str) or DIGEST_RE.fullmatch(artifact_digest) is None:
        errors.append("verified release artifact digest must use sha256:<64 lowercase hex>")

    canonical_name = ""
    if _is_positive_int(publisher_run_id) and _is_positive_int(publisher_run_attempt):
        canonical_name = canonical_verified_release_artifact_name(
            publisher_run_id,
            publisher_run_attempt,
        )
        if artifact_name != canonical_name:
            errors.append(
                "verified release artifact name must equal canonical current "
                f"publisher-attempt name {canonical_name}"
            )
    elif not isinstance(artifact_name, str) or not artifact_name:
        errors.append("verified release artifact name must be a non-empty string")

    if not isinstance(artifact_metadata, dict):
        errors.append("verified release artifact API metadata must be a JSON object")
        return errors

    metadata_id = artifact_metadata.get("id")
    if not _is_positive_int(metadata_id) or metadata_id != artifact_id:
        errors.append("verified release artifact API identity mismatch")

    metadata_name = artifact_metadata.get("name")
    if not canonical_name or metadata_name != canonical_name:
        errors.append(
            "verified release artifact API name does not match current publisher attempt"
        )

    if artifact_metadata.get("expired") is not False:
        errors.append("verified release artifact is expired or has invalid expiration state")
    ...
    if not isinstance(workflow_run, dict):
        errors.append("verified release artifact workflow_run metadata is missing")
    else:
        workflow_run_id = workflow_run.get("id")
        if not _is_positive_int(workflow_run_id) or workflow_run_id != publisher_run_id:
            errors.append(
                "verified release artifact does not belong to the current publisher run"
            )

        workflow_repository_id = workflow_run.get("repository_id")
        if (
            not _is_positive_int(workflow_repository_id)
            or workflow_repository_id != repository_id
        ):
            errors.append(
                "verified release artifact repository ID does not match current repository"
            )
        ...
        if (
            ...
            errors.append(
                "verified release artifact head repository ID does not match current repository"
            )
        ...
        if not isinstance(workflow_head_sha, str) or SHA_RE.fullmatch(workflow_head_sha) is None:
            errors.append(
                "verified release artifact publisher head SHA is missing or malformed"
            )
        elif workflow_head_sha != publisher_sha:
            errors.append("verified release artifact publisher head SHA mismatch")
