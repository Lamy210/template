...
[[ -d "${GITHUB_WORKSPACE}/${WORKING_DIRECTORY}" ]] || exit 2
...
if [[ -n "${PROJECT_PATH}" ]]; then
  ...
  [[ -d "${GITHUB_WORKSPACE}/${WORKING_DIRECTORY}/${PROJECT_PATH}" ]] || exit 2
else
  safe_relative_path "${WORKSPACE_PATH}" || exit 2
  [[ -d "${GITHUB_WORKSPACE}/${WORKING_DIRECTORY}/${WORKSPACE_PATH}" ]] || exit 2
fi
...
[[ ! -e "${GITHUB_WORKSPACE}/artifacts/visual" ]] || exit 2
[[ ! -e "${E2E_RESULT_BUNDLE}" ]] || exit 2
